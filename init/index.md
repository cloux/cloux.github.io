...
Init is the first process started by the kernel, and continues running until the system is shut down. In other words, if it stops, for any reason, your system stops working with it. Every init system out there has to do a few important things: initialize system at boot, manage services while the system is running, and finally shutdown or reboot the system.

Init commands were historically designed to run serially, in a sequence. Later as computer hardware became more powerful, and software parallelization became necessary as the means of taking advantage of this trend. (See [Amdahl's-Gustaffson's trend](https://software.intel.com/en-us/articles/amdahls-law-gustafsons-trend-and-the-performance-limits-of-parallel-applications) and limits of parallelization study.) This trend was also applied to modern designs of init systems. Unfortuantely, absolute boot parallelization is impossible due to dependencies. Some tasks cannot be started, unless others finished. A logfile cannot be written until the filesystem is mounted, the filesystem cannot be mounted until it was checked for errors, filesytems cannot be checked until all devices are available... Thus it was necessary to define and implement these relations, adding some code overhead and complexity to init...

Running computer tasks in parallel is generally much more complicated than simple serial execution. It is something that operating systems were designed to do, and even they used to suck at it. And when these tasks depend on each others results, the problem gets much harder and the solution more complex. For modern, powerful systems where boot speed is absolutely crucial and even has higher priority than stability or reliability, a parallel boot stage should definitely be considered as an option. How much speedup are we talking about? A combined sequential-parallel boot of [runit](#runit) init, on a slow single-CPU core, was measured at [less than 7 seconds](https://github.com/cloux/aws-devuan#ec2-linux-ami-comparison). That could be pushed further down to around 5 seconds. If you **need** to be faster, you might need a heavily parallelized init like [systemd](#systemd). Other inits combine the sequential and parallel approach, trying to get the best of both worlds. All approaches have advantages and disadvantages:
